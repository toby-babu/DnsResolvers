...
public class mydig {
    ...
    fetchDNSQuery(domainToQuery, nameServerList.get(nsIndex), typeOfRecord);
        //fetchDNSQuery("www.eurid.eu.", nameServerList.get(nsIndex), Type.A);
    }
     ...
     */
    private static void fetchDNSQuery(String dnsquery, String nsIp, int requestedType) throws IOException {
     ...
     * @return
     */
    private static String resolveQuery(String dnsquery, String nsIp, TypeOfQuery queryType, int requestedType) throws IOException {
        ...
        try {
            ...
            for(Record record: ansrecords) {
                switch (record.getType()) {
                    ...
                    case Type.CNAME:
                        ...
                        nextCName.add(currentName.toLowerCase());
                        break;
                    case Type.MX:
                        if (ipflag == Boolean.FALSE) {
            ...
            for(Record record: authority) {
                switch (record.getType()) {
                    case Type.A:
                        String currentIp = ((ARecord) record).getAddress().toString();
                        currentIp = currentIp.substring(currentIp.lastIndexOf('/') + 1);
                        nextNStoQuery.add(currentIp.toLowerCase());
                        break;
                    case Type.NS:
                        ...
                        break;
                    case Type.SOA:
                        if ((queryType == TypeOfQuery.NS || queryType == TypeOfQuery.ORIGINAL) && (requestedType == Type.NS || requestedType == Type.MX)) {
                ...
                for(Record record: additionalrecords) {
                    switch (record.getType()) {
                        case Type.A:
                            ...
                            currentName = currentName.substring(0, currentName.length() - 1);
                            if (nextNS.contains(currentName.toLowerCase())) {
                                String currentIp = ((ARecord) record).getAddress().toString();
                                currentIp = currentIp.substring(currentIp.lastIndexOf('/') + 1);
                                nextNStoQuery.add(currentIp.toLowerCase());
                            }
                            break;
                        case Type.NS:
                            nextNS.add(record.getName().toString().toLowerCase());
                            break;
            ...
            // Try the next name server during time out
            if (nsIndex < 12) {
                nsIndex++;
